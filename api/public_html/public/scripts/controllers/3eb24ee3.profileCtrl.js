    ...
    .controller('profileCtrl', ['$rootScope', '$scope', '$http',
        ...
        function($rootScope, $scope, $http, alertService,
            ...
            $scope.setUpNewReps = function() {
                ...
                for (var k = 0; k < 3; k++) {
                    if ($scope.previousSenators[k].id == $scope.reps[i].id) {
                        // do nothing
                    } else {
                        // there's a new rep!

                        // set new rep's ID
                        $scope.previousSenators[k].id = $scope.reps[i].id;

                        // clear out previous rep's data
                        $scope.previousSenators[k].disagree = 0;
                        $scope.previousSenators[k].noVote = 0;

                    }
                }

                console.log('finished reps');
                console.log($scope.previousReps);
            }
