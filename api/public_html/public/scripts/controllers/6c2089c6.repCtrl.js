    ...
    function($scope, $http, $rootScope, $location, $filter, $timeout) {
        ...
        $http.get($rootScope.urlRoot + '/user/reps').success(function(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                newRepWorth[i] = data[i].worthiness;
                newRepWorth[i] = $filter('number')(newRepWorth[i], 1);
            }
            $scope.reps = [{
                "firstName": data[0].first_name,
                "lastName": data[0].last_name,
                "bioguide_id": data[0].bioguide_id,
                "repworthiness": newRepWorth[0],
                "role": "Senator"
            }, {
                "firstName": data[1].first_name,
                "lastName": data[1].last_name,
                "bioguide_id": data[1].bioguide_id,
                "repworthiness": newRepWorth[1],
                "role": "Senator"
            }, {
                "firstName": data[2].first_name,
                "lastName": data[2].last_name,
                "bioguide_id": data[2].bioguide_id,
                "repworthiness": newRepWorth[2],
